...
<div class="py-4">
    <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
        <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
            ...
            <li class="breadcrumb-item active">tracking List</li>
        </ol>
    </nav>
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-md-12 col-lg-12">
                    <div id="tracking-pre"><button class="btn btn-info mb-3" id="btnAdd">Tambah Tracking Layanan</button></div>
                    <div id="tracking">
                        ...
                        <div class="tracking-list">
                            ...
                            <div class="tracking-item">
                                ...
                                <div class="tracking-date">{{$list->tgl_layanan->translatedFormat('d F Y')??'-'}}</div>
                                <div class="tracking-content">Status : {{$list->status->nama}}
                                    <span>keterangan:{{$list->keterangan??'-'}}</span>
                                     @if ($list->url_file!= null)
                                        <a href="{{ $list->url_file??'-' }}" target="_blank" class="btn btn-primary btn-sm">Lihat File</a>
                                    @endif
                                     <span>
                                        <button class="btn btn-sm btn-info btn-edit" data-id="{{ $list->id }}">Edit</button>
                                         <button class="btn btn-sm btn-danger btn-delete" data-id="{{ $list->id }}">Hapus</button>
                                    </span>
                                </div>
                            </div>
                            @endforeach
                            <div class="tracking-item">
                                ...
                                <div class="tracking-date">{{$data->tgl_pengajuan->translatedFormat('d F Y')??'-'}}</div>
                                <div class="tracking-content">Pengajuan layanan : {{$data->jenisLayanan->nama}} <span>keterangan:{{$data->keterangan??'-'}}</span>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="formLayanan">
            ...
            <input type="hidden" id="detail_layanan_id" name="detail_layanan_id" value="{{$data->id}}">
...
<script>
    ...
    $(function () {
        ...
        $('#btnSave').on('click', function () {
            ...
            let url = id ? `/tracking-update/${id}` : '{{ route('tracking.store') }}';
        ...
        // ...existing code...
            ...
            $(document).on('click', '.btn-edit', function () {
                let id = $(this).data('id');
                $.get(`/tracking-show/${id}`, function (data) {
                    $('#layanan_id').val(data.id);
                    $('#status_id').val(data.status_id); // set status if available
                    $('#tgl_layanan').val(data.tgl_layanan); // match your form field
